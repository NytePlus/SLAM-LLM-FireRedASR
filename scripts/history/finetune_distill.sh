...
dataset=slidespeech
task=asr
train_scp_file_path=${DATA_DIR}/${dataset}/train_95/history
dev_scp_file_path=${DATA_DIR}/${dataset}/dev_oracle_v1/history
...
export DISTILL_IGNORE=$'<|im_start|>user: '
output_dir=${EXP_DIR}/distill-$llm_name-$projector-$(date +"%Y%m%d-%H%M")-$dataset
hydra_args="
...
++model_config.attn_distill_weight=100 \
++model_config.attn_distill_layers=null \
++model_config.attn_distill_context_ratio=0.75 \
